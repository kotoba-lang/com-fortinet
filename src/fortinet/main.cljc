(ns fortinet.main
  "Kotodama WASM entrypoint for the Fortinet clean-room actor (L4) — Clojure port.
  ...
  facts (`fortinet.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fortinet")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"fortinet.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fortinet-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fortinet-compat/ready)
